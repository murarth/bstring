// This file is a modified version of the Rust implementation of the
// str pattern API.
//
// The original Rust Project copyright header follows.
...
/// would match at index `1` in the byte string `"baaaab"`.
///
...
/// matches of a pattern starting from the front (left) of a byte string.
///
...
///
/// This trait provides methods for searching for non-overlapping
/// matches of a pattern starting from the back (right) of a byte string.
///
/// It will be implemented by associated `Searcher`
...
/// # Examples
///
/// `u8::Searcher` is a `DoubleEndedSearcher` because searching for a
/// `u8` only requires looking at one at a time, which behaves the same
/// from both ends.
///
/// `(&bstr)::Searcher` is not a `DoubleEndedSearcher` because
/// the pattern `"aa"` in the haystack `"aaa"` matches as either
/// `"[aa]a"` or `"a[aa]"`, depending from which side it is searched.
pub trait DoubleEndedSearcher<'a>: ReverseSearcher<'a> {}

/////////////////////////////////////////////////////////////////////////////
// Impl for a ByteEq wrapper
/////////////////////////////////////////////////////////////////////////////
    ...
    fn next(&mut self) -> SearchStep {
        match self.searcher {
            StrSearcherImpl::Empty(ref mut searcher) => {
                // empty needle rejects every byte and matches every empty string between them
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                // TwoWaySearcher produces valid *Match* indices that split at byte boundaries
